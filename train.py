...
def main(args):
    ...
    images = torch.zeros(1, 3, 224, 224).to(device)#equal to the input size of images
    tb_writer.add_graph(model, images, verbose=False)

    if args.weights != "":
        assert os.path.exists(args.weights), "weights file: '{}' not exist.".format(args.weights)
        weights_dict = torch.load(args.weights, map_location=device)
        # delete the weights we don't need
    ...
    if args.freeze_layers:
        for name, para in model.named_parameters():
            # freeze the weights except head and pre_logits
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data-path', type=str,
                        default=r"F:/course/tree_identification/dataset/fall_images/DJI_20211116__FALL_dataset")
